...
| Configuration (`config.py`) | Declaring every lever the whole pipeline will ever have — including ones no stage uses yet — and reading them from `config.toml`, with in-code defaults so an absent key or file is never a crash. Paths are levers too: `data_dir` for the corpus cache and `alias_table_path` for the hand-authored alias table, which is why the two can be moved independently. One lever has no default and cannot get one: `extraction_model` is empty until `config.toml` names a model, and extraction REFUSES rather than choosing. **The VALUE is not configuration's to decide** — `docs/DESIGN.md` §6 fixes it by owner ruling (Opus 5 throughout, low effort for extraction), with reasoning effort as the cost lever rather than the model. It is a key rather than a constant because R17 makes the cost levers configuration and because the calibration record must name the model it measured: a chars-per-token factor measured against one model does not transfer to another. |
