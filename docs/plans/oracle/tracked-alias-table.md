...
- **It costs you:** one line in `config.toml`; one edited assertion in a
  blind-written config test, because slice 2 flips the default that slice 1
  pins; and a test that runs `git` as a subprocess, which the offline suite has
  none of today.
...
moves the file and flips its default. Slice 1 shares `commands/select.py` with
`docs/plans/oracle/refuse-on-missing-artifact.md`; the two are never built in
parallel.
...
`config.toml` and `docs/architecture.md`. A cut that moved the file first and
re-homed the loaders afterwards was rejected: it breaks both stages in between,
and R1007 requires the move and the default to land together anyway.
...
- **What the tests must pin.** In `tests/test_config.py`: a `config.toml`
  carrying `alias_table_path` yields that `Path`; a file omitting it yields
...
- **The location guard is a module of its own** — the new
  ...
  `tests/test_aliases.py`, because three
  in-flight plans edit that file, and this test shares nothing with it but the
  subject. It asserts, against the real checkout:
...
- **Building in parallel with `refuse-on-missing-artifact`.** Slice 1 rewrites
  `commands/select.py`'s `_missing`, which that plan's slice 2 also rewrites.
  Whichever lands second carries the rebase; one pipeline pull request in flight
  at a time makes that a sequencing note rather than a risk.
